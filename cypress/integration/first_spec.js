...
describe('Landing tests', () => {
    ...
    it('tests if all fields are visible and enabled, tests if all titles are proper', () => {
        ...
        cy.get(emailSendingPageSelectors.name_field_title).should('be.visible').invoke('text').should('eq', emailSendingPageUtils.first_name_copy)  
        cy.get(emailSendingPageSelectors.name_field_title).should('be.visible').should('not.to.be.disabled')
        cy.get(emailSendingPageSelectors.surname_field_title).should('be.visible').invoke('text').should('eq', emailSendingPageUtils.surname_copy)  
        cy.get(emailSendingPageSelectors.surname_field).should('be.visible').should('not.to.be.disabled')
        cy.get(emailSendingPageSelectors.newsletter_type_field_title).should('be.visible').invoke('text').should('eq', emailSendingPageUtils.newsletter_type_copy)  
