export var emailSendingPageSelectors = {
    
    email_field: '#newsletter_email',
    email_field_title: 'label[title="E-mail"]',
    name_field: '#newsletter_name',
    name_field_title: 'label[title="First name"]',
    surname_field: '#newsletter_surname',
    surname_field_title: 'label[title="Surname"]',
    newsletter_type_field: '#newsletter_newsType',
    newsletter_type_field_title: 'label[title="Newsletter type"]',
    start_date_field: 'newsletter_startDate',
    start_date_field_title: 'label[title="Start date"]',
    end_date_field: '#newsletter_endDate',
    end_date_field_title: 'label[title="End date"]',
    sex_buttons: '#newsletter_Sex > .ant-radio-wrapper > .ant-radio',
    sex_buttons_title: 'label[title="Sex"]',
    agreement_checkmark: '#newsletter_agreement',
    agreement_link: 'a',
    submit_button: 'button[type=submit]',
    footer: 'footer'
}